    ...
    override suspend fun doWork(): Result {
        return try {
            ...
            if (runAttemptCount < Constants.MAX_WORK_RETRY_ATTEMPTS) {
                ...
                // Keep periodic work alive after a transient/OEM failure. A failed periodic
                // request can stop future executions until the app is opened again.
                Log.w(TAG, "Giving up this run after retries; periodic schedule remains active")
                Result.success()
            }
        }
    }
